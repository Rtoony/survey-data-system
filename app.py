...
@app.route('/api/material-standards')
def get_material_standards():
    """Get all material standards for dropdown selection"""
    try:
        ...
        query = f"""
            SELECT material_id, material_code, material_name, category, description,
                   typical_uses, cost_factor, is_active, created_at, updated_at
            FROM material_standards
            {where_sql}
            ORDER BY material_name
        """
...
@app.route('/api/category-codes')
def get_category_codes():
    """Get all category codes for dropdown selection"""
    try:
        active_only = request.args.get('active_only', 'true') == 'true'
...
# ===== DISCIPLINE CODES API =====

@app.route('/api/discipline-codes')
def get_discipline_codes():
    """Get all discipline codes for dropdown selection"""
    try:
        active_only = request.args.get('active_only', 'true') == 'true'
        ...
        """
        results = execute_query(query)
        return jsonify({'disciplines': results})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
