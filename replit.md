...
- **Z-Value Stress Test (`/tools/z-stress-test`):** Interactive web interface and CLI tool for proving XYZ coordinate preservation over multiple import/export cycles with comprehensive SRID support. **VERIFIED: All three coordinate systems (LOCAL, State Plane, WGS84) achieve perfect 0.000000 ft precision over 5+ cycles for ALL entity types.** Web UI features configurable test parameters (cycles, coordinate system, tolerance), real-time progress tracking, per-axis error dashboard (ΔX, ΔY, ΔZ), visual error charts with pass/fail badges, and downloadable JSON reports. CLI version (`scripts/z_stress_harness.py`) generates auditable reports with SHA256 hashes, per-cycle error metrics (max/avg/per-axis deltas), and canonical test fixtures (Z=0 flat pads, sloped pipes, survey points, large coordinates, sub-millimeter precision, multiple LINE entities). **Universal Spatial Matching:** Test harness uses centroid-based entity pairing to handle DXF file reordering across ALL entity types (POLYLINE, LINE, POINT, 3DFACE, etc.), ensuring accurate vertex-level comparison regardless of entity ordering changes between cycles. This resolves Civil 3D triangulated surface comparison issues where 100s of 3DFACE entities reorder unpredictably. Supports three coordinate systems: SRID 0 (LOCAL CAD in ft), SRID 2226 (CA State Plane Zone 3 in ft), and SRID 4326 (WGS84 in degrees/meters). SRID-specific tolerances: ±0.001 ft for LOCAL/STATE_PLANE, ±1e-7° XY and ±0.01m Z for WGS84. Fixtures use SRID-appropriate coordinates: raw feet for LOCAL, ~6M ft offsets for State Plane, and degrees/meters with SF area base for WGS84. Includes coordinate sanity validation for uploaded files. Exit codes reflect pass/fail status for CI integration.
...
- **Batch Point Import Tool (`/tools/batch-point-import`):** Survey point import tool for PNEZD (Point Number, Northing, Easting, Elevation, Description) text files. Features coordinate system selection with automatic transformation via pyproj, drawing assignment, conflict detection, user-selectable import/update/skip actions, preview grid, and batch processing. Points are stored with PostGIS 3D geometry (ST_MakePoint) in SRID 2226, preserving elevation as Z-coordinate. Backend module (`batch_pnezd_parser.py`) handles parsing, validation (requires all five fields), coordinate transformation, and database operations with parameterized queries.
- **Survey Code Library Manager (`/tools/survey-codes`):** AI-first survey point code system replacing arbitrary text descriptions with structured, machine-parsable codes that drive automatic CAD generation. **Database Schema:** `survey_code_library` (62 comprehensive codes), `survey_code_attributes` (attribute definitions), `survey_line_segments` (auto-generated polylines), and extended `survey_points` with code parsing columns. **Features:** CRUD interface with stats dashboard, multi-criteria filtering (discipline, category, connectivity type, favorites), CSV export for data collector import, and comprehensive user documentation (`SURVEY_CODE_SYSTEM_GUIDE.md`). **Code Structure:** Hierarchical format `[DISCIPLINE]-[CATEGORY]-[FEATURE_TYPE]` with 6 connectivity types (NODE, LINE, EDGE, POINT, BREAK, CLOSE) controlling auto-connection behavior, block insertion, and layer template generation. **Coverage:** 62 active codes spanning utilities (storm, sanitary, water, gas, electric, telecom), site features (roads, parking, walks, fences, buildings), topographic data, vegetation, and control points. **Field Workflow:** Field crews use standardized codes in data collectors; system automatically generates CAD entities, creates polylines from sequential shots, inserts blocks at NODE locations, and applies template-based layer naming. **Export:** CSV download provides complete code library with 16 fields (code, display name, description, connectivity, geometry output, auto-connect flags, layer template, usage tracking) formatted for data collector import.
- **Survey Code Testing Interface (`/tools/survey-code-tester`):** Comprehensive 4-tab testing and validation system for survey codes powered by `survey_code_parser.py` module. **Tab 1 - Single Code Test:** Real-time code parsing with property display grid (discipline, category, feature_type, connectivity, geometry_output, etc.), layer name preview showing template resolution (e.g., CIV-UTIL-STORM-MH → CIV-UTIL-STORM-MH-EXIST-SYMB), CAD output preview (block/polyline/point), and phase override dropdown. **Tab 2 - Field Shot Simulator:** Dynamic shot list for simulating field sequences with point number, code, northing, easting, elevation inputs; processes multi-point sequences to demonstrate connectivity behavior (which points auto-connect based on LINE/EDGE connectivity rules); displays results showing polylines created, blocks inserted, warnings, and visual connectivity diagram. **Tab 3 - Batch Validator:** Dual-mode validation (manual entry or CSV upload) with 5,000 code / 2 MB limits; manual textarea accepts one code per line; CSV upload features drag-and-drop zone, file validation, UTF-8 encoding checks, comment skipping (#), and auto-extraction from first column; displays stats dashboard (total/valid/invalid counts), detailed per-code results with error messages, and Download Results button for RFC4180-compliant CSV export with tokenized download endpoint (`/api/survey-codes/batch-validate-download/<token>`). **Tab 4 - Connectivity Rules:** Interactive documentation of all 6 connectivity types with descriptions, examples, auto-connect behavior, geometry output, and visual icons. **Backend:** Server-mediated batch pipeline with shared `process_batch_validation()` helper function, `/api/survey-codes/batch-validate` (manual entry), `/api/survey-codes/batch-validate-upload` (CSV upload with security checks), and `/api/survey-codes/batch-validate-download/<token>` (CSV download with CSV injection prevention). **Security:** File size/count limits, MIME validation, UTF-8 checks, newline/quote sanitization in CSV output, XSS prevention via escapeHtml(), token-based download. System designed as Phase 2 foundation with future phases planned for import tool with auto-connectivity and DXF export integration.
