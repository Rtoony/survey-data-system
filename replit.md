...
- **November 9, 2025**: CURATED LAYER EXAMPLES CATALOG - Created comprehensive visual reference guide with 45 curated layer name examples covering all 8 geometry types (LN=8 examples, PT=10, PG=9, TX=5, 3D=3, BK=4, HT=3, DIM=3). Replaced brute-force example generation with strategic catalog ensuring realistic, balanced representation across all disciplines (CIV, SURV, LAND, ANNO) and categories. Layer Standards Reference page (`/standards/reference`) now shows filterable examples for all geometry types with full metadata (DXF entity types, database tables, phase colors). Architect-approved implementation provides fast API response and comprehensive test data source for users building sample datasets. Added direct "CAD Standards Portal" and "CAD Standards Vocabulary" links to sidebar navigation for easy access.
- **November 9, 2025**: CAD LAYER NAMING STANDARDS & DATABASE-DRIVEN CLASSIFIER - Created comprehensive CAD Layer Naming Standards documentation (`docs/CAD_LAYER_NAMING_STANDARDS.md`) defining the pattern syntax: `[DISCIPLINE]-[CATEGORY]-[OBJECT_TYPE]-[PHASE]-[GEOMETRY]` with examples for all object types (gravity/pressure pipes, survey, landscape, alignments, BMPs, blocks, annotations). Implemented database-driven LayerClassifierV3 (`standards/layer_classifier_v3.py`) that queries `discipline_codes`, `category_codes`, `object_type_codes`, `phase_codes`, and `geometry_codes` tables instead of using hardcoded regex patterns. Classifier extracts diameters from layer names (e.g., "12STORM" → diameter=12), detects network mode (gravity vs. pressure), supports legacy formats ("12IN-STORM", "STORM-PROPOSED"), and includes `reload_codes()` method for cache invalidation. Populated missing utility object types in database (STORM/SANIT/MH/CB/INLET/CLNOUT in STOR category; WATER/RECYC/HYDRA/VALVE/METER in ROAD category; CTRL/TOPO/BNDY in survey categories). Achieved 100% test success rate (27/27 scenarios) with architect approval. Added Standards Reference Guide link to sidebar navigation for easy access to naming structure documentation.
- **November 9, 2025**: CONTROLLED VOCABULARY SYSTEM - Implemented comprehensive Categories and Disciplines management system replacing free-form text entry with controlled dropdowns. Created `standard_categories` and `standard_disciplines` tables with junction tables for many-to-many mappings to standard types, seeded with 10 CAD categories (CIVIL, SURVEY, UTILITIES, LANDSCAPE, GRADING, DRAINAGE, ADA, TRAFFIC, ANNOTATION, GENERAL) and 9 disciplines (CIVIL, SURVEY, LANDSCAPE, ELECTRICAL, MECHANICAL, PLUMBING, STRUCTURAL, ARCHITECTURAL, GENERAL). Built full CRUD managers at `/data-manager/categories` and `/data-manager/disciplines` with add/edit/delete operations. Updated Blocks, Details, and Abbreviations managers to use category/discipline dropdowns instead of text inputs, with API-driven population via filtered endpoints (`/api/categories?standard_type=blocks`, `/api/disciplines?standard_type=details`). Ensures data consistency across all CAD standards objects.
