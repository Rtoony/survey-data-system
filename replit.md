...
- **Z-Value Stress Test (`/tools/z-stress-test`):** Interactive web interface and CLI tool for proving Z-value preservation over multiple import/export cycles. Web UI features configurable test parameters (cycles, SRID, tolerance), real-time progress tracking, visual error charts, and downloadable JSON reports. CLI version (`scripts/z_stress_harness.py`) generates auditable reports with SHA256 hashes, per-cycle error metrics (max/avg/z-delta), and canonical test fixtures (Z=0 flat pads, sloped pipes, large coordinates, sub-millimeter precision). Supports both SRID 0 (local CAD) and EPSG:2226 (CA State Plane). Exit codes reflect pass/fail status for CI integration.
