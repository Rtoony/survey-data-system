...
- **Batch Block Import Tool (/tools/batch-block-import):** Productivity tool for batch importing block definitions from multiple DXF files. Features drag-and-drop multi-file upload, automatic block extraction and categorization (9 category patterns: title blocks, symbols, landscape, vehicles, utilities, etc.), SVG preview generation, existing block detection, conflict resolution (import/update/skip actions), search and filter capabilities, and batch selection controls. Supports importing entire CAD block libraries in minutes vs hours of manual entry.
