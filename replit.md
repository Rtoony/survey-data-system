...
- **Gravity Pipe Network Editor (/gravity-network-editor):** Modernized interactive editor using Entity Viewer pattern with SVG rendering on black background. Features real-time network visualization (color-coded Storm/Sanitary pipes, structures as circles), bidirectional selection sync (canvas clicks highlight table rows, table clicks highlight canvas entities), auto-fit viewport with WGS84 coordinate transformation, and legend/info panels. Preserves all existing editing capabilities (inline table edits, Auto-Connect, Save Changes) while adding visual context via `/api/pipe-networks/<network_id>/viewer-entities` endpoint.
