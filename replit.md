...
- **Z-Value Stress Test (`/tools/z-stress-test`):** Interactive web interface and CLI tool for proving XYZ coordinate preservation over multiple import/export cycles with comprehensive SRID support. Web UI features configurable test parameters (cycles, coordinate system, tolerance), real-time progress tracking, per-axis error dashboard (ΔX, ΔY, ΔZ), visual error charts with pass/fail badges, and downloadable JSON reports. CLI version (`scripts/z_stress_harness.py`) generates auditable reports with SHA256 hashes, per-cycle error metrics (max/avg/per-axis deltas), and canonical test fixtures (Z=0 flat pads, sloped pipes, large coordinates, sub-millimeter precision). Supports three coordinate systems: SRID 0 (LOCAL CAD in ft), SRID 2226 (CA State Plane Zone 3 in ft), and SRID 4326 (WGS84 in degrees/meters). SRID-specific tolerances: ±0.001 ft for LOCAL/STATE_PLANE, ±1e-7° XY and ±0.01m Z for WGS84. Fixtures use real-world coordinates (~6M ft offsets for State Plane, ~-122.4° lon / 37.8° lat for WGS84). Includes coordinate sanity validation for uploaded files. Exit codes reflect pass/fail status for CI integration. Known limitation: DXFImporter's ProjectMappingRegistry applies persistent local-grid offsets during imports, requiring bypass mechanism for pure stress test scenarios.
